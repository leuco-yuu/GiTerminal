...
class IconButton(QToolButton):
    def __init__(self, text: str = "", tooltip: str = "", icon_path: str | None = None) -> None:
        ...
        self.original_icon_path = str(icon_path) if icon_path else None
    ...
    def apply_icon_theme(self, mode: str, accent: str) -> None:
        if not self.original_icon_path:
            return
        src = Path(self.original_icon_path)
        if not src.exists():
            return
        foreground = "#f3f4f6" if mode == "dark" else "#1f2937"
        svg = src.read_text(encoding="utf-8")
        svg = svg.replace("#d4d4d4", foreground).replace("#5fb3f3", accent)
        cache_dir = Path(tempfile.gettempdir()) / "git_terminal_themed_icons"
        cache_dir.mkdir(parents=True, exist_ok=True)
        out = cache_dir / f"{mode}_{accent.strip('#')}_{src.name}"
        out.write_text(svg, encoding="utf-8")
        self.setIcon(QIcon(str(out)))
        self.setIconSize(QSize(22, 22))
...
class ActivityBar(QFrame):
    ...
    def apply_icon_theme(self, mode: str, accent: str) -> None:
        for button in [*self.buttons, self.account_button, self.settings_button]:
            button.apply_icon_theme(mode, accent)
...
class TopCommandBar(QFrame):
    def __init__(self) -> None:
        ...
        # No command-center/search box: the top-right area is reserved for
        # explicit layout controls, matching VS Code's sidebar/panel toggles.
        self.command_center = QLineEdit(self)
...
class SidePanel(QFrame):
    def __init__(self, title: str, side: str = "left", min_size: int = 220) -> None:
        ...
        self.header_layout.setContentsMargins(6, 0, 4, 0)
        ...
        self.collapse_button.setMinimumSize(0, 20)
        self.collapse_button.setMaximumSize(20, 20)
        self.collapse_button.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Fixed)
