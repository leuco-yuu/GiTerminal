...
- 底部输入栏改为真实命令终端：用户可以输入任意 shell 命令，不再限定 Git 子命令；`git ...` 命令仍会走 Git 安全执行和状态刷新。
- 底部日志栏新增最大化按钮，可展开占领整个中央工作区域，也可以恢复或收起。
- 底部栏移除 “TERMINAL” 标题文字，日志区域直接占用底部空间；折叠/最大化控制移动到底部输入栏右侧。
- 原需要 QInputDialog 输入的操作改为终端上方的内联提示区，不再弹出独立输入弹窗，包括创建分支、远程、标签、GitHub 仓库、push upstream、远程删除分支、危险命令确认等。
- 左下角设置菜单继续按程序窗口底部对齐。
